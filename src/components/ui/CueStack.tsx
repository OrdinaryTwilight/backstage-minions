      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        ...
        <tbody>
          {cues.map((cue, idx) => {
            ...
            let flashClass = "";
            if (result) flashClass = result.hit ? "row-flash-hit" : "row-flash-miss";
            ...
            return (
              ...
              >
                ...
                <td style={{ textAlign: 'right', fontWeight: 'bold' }}>
                  {result ? (
                    <span style={{ color: result.hit ? 'var(--bui-fg-success)' : 'var(--bui-fg-danger)' }}>
                      {result.hit ? "DONE" : "FAIL"}
                    </span>
                  ) : isCurrent ? (
                    <span className="animate-flicker">STANDBY</span>
                  ) : "-"}
                </td>
              </tr>
            );
