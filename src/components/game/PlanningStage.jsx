...
import { LIGHT_TYPES, PLOT_GRID_COLS, PLOT_GRID_ROWS } from "../../data/gameData";
...
export default function PlanningStage({ onComplete }) {
  ...
  return (
    <div>
      ...
      <p>
        Build your lighting plot. Select a fixture type, then tap the grid to place it.
        <br />
        The Stage Preview on a real board would show you the effect — here, fill the plot
        before submitting to the Stage Manager.
      </p>
      ...
      <div style={{ marginBottom: "1rem", display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
        {LIGHT_TYPES.map(l => (
          <button
            ...
            style={{ padding:"0.4rem 0.8rem", borderRadius:"8px", fontSize:"0.82rem",
              border:`2px solid ${selectedType === l.id ? l.color : "var(--border)"}`,
              background: selectedType === l.id ? `${l.color}22` : "var(--surface2)",
      ...
      {/* Grid */}
      <div style={{ display: "grid", gridTemplateColumns: `repeat(${PLOT_GRID_COLS}, 1fr)`, gap: "0.25rem", marginBottom: "1rem" }}>
        {grid.map((cell, i) => {
          const lt = cell ? LIGHT_TYPES.find(t => t.id === cell.typeId) : null;
          return (
            <button
              key={i}
              onClick={() => placeLight(i)}
              title={lt ? lt.label : "Empty"}
              style={{ aspectRatio: "1", padding: "0", cursor: "pointer", background: lt ? `${lt.color}44` : "var(--surface2)", border: "1px solid var(--border)", fontSize: "1.2rem" }}>
              {lt ? (
                lt.icon
              ) : (
                "·"
      ...
      {!submitted ? (
        <button onClick={submit} style={{ cursor: "pointer", padding: "0.75rem 1.5rem", marginTop: "1rem" }}>
        ...
        <div style={{ marginTop: "1rem", padding: "1rem", background: "var(--surface2)", borderRadius: "8px" }}>
          ...
          <div>
            Fixtures placed: {grid.filter(Boolean).length}
          </div>
          <div>
            Score earned: +{reportScore} pts
          </div>
          <p>
            {reportScore >= 80
          ...
          <button onClick={onComplete} style={{ cursor: "pointer", marginTop: "1rem" }}>
            Continue
          </button>
        </div>
      )}
